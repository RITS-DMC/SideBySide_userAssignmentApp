...
function getClientCredentialsTokenByTenant(correlationId, tenant, credentials, zoneInfo, cb) {
  const config = createConfigObject(credentials.uaa || credentials, correlationId, XSSEC_TIMEOUT);
  return xssec.requests.requestClientCredentialsToken(tenant, config, null, zoneInfo && zoneInfo.apptid || null, (err, accessToken, tokenInfo) => {
...
async function getApp2AppToken(idToken, credentials, IASDependencyName, correlationId) {
  return new Promise((resolve, reject) => {
    credentials.url = credentials.tokenEndPoint ? credentials.tokenEndPoint.replace('/oauth2/token','') : credentials.url;
    const config = {
      correlationId: correlationId,
      resource: 'urn:sap:identity:application:provider:name:' + IASDependencyName,
      type: 'IAS',
      credentials: credentials,
      timeout: XSSEC_TIMEOUT
    };
    xssec.requests.requestUserToken(idToken, config, null, null, null, credentials.apptid, (err, token, tokenJson) => {
      if (err) {
        const errorSource = err.response && err.response.data && err.response.data.error
          ? ' - ' + err.response.data.error + ' - ' + err.response.data.error_description
          : err.response && err.response.data ? ' - ' + err.response.data : '';
        err.message = 'Failed to exchange IAS token: ' + err.message + errorSource;
        reject(err);
      } else {
        resolve(tokenJson);
      }
...
function exchangeToken(loginToken, correlationId, externalServiceCredentials, cb) {
  try {
    ...
    const config = createConfigObject(externalServiceCredentials, correlationId, XSSEC_TIMEOUT);
    xssec.requests.requestUserToken(loginToken, config, null, null, subdomain, apptid, (err, token, tokenJson) => {
      if (err){
...
function getClientCredentialsToken(subdomain, correlationId, credentials, apptid) {
  return new Promise((resolve, reject) => {
    const config = createConfigObject(credentials, correlationId, XSSEC_TIMEOUT);
    xssec.requests.requestClientCredentialsToken(subdomain, config, null, apptid, (err, token, tokenJson) => {
