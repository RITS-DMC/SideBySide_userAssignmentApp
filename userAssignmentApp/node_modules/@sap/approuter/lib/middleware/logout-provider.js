...
module.exports = {
  ...
  validateLogoutToken: async function (token) {
    ...
    return new Promise((resolve, reject) => {
      xssec.createSecurityContext(token, credentials, 'IAS', function (err, _context, tokenInfo) {
        if (err) {
          return reject(err);
        } else {
          const payload = tokenInfo.getPayload();
          if (payload.nonce) {
            return reject(new Error('Nonce is not allowed in logout token'));
          }

          const logoutEvent = payload.events && payload.events['http://schemas.openid.net/event/backchannel-logout'];
          if (!logoutEvent) {
            return reject(new Error('Event claim is missing in logout token'));
          }

          const sid = payload.sid;
          if (!sid) {
            return reject(new Error('Session ID is missing in logout token'));
          }
          resolve(sid);
        }
      });
